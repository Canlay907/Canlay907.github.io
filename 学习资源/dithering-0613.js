...
const sevenColorPalette = [
  ...
  { name: "红色", r: 250, g: 0, b: 0, value: 0x04 },
...
const epdRealColors = {
  ...
  sevenColor: [
    ...
    { name: "白色", realR: 255, realG: 240, realB: 200, r: 255, g: 255, b: 255, value: 0x01 },
    { name: "绿色", realR: 41, realG: 255, realB: 20, r: 0, g: 255, b: 0, value: 0x02 },
    { name: "蓝色", realR: 0, realG: 0, realB: 240, r: 0, g: 0, b: 255, value: 0x03 },
    { name: "红色", realR: 227, realG: 0, realB: 0, r: 255, g: 0, b: 0, value: 0x04 },
    { name: "黄色", realR: 255, realG: 255, realB: 0, r: 255, g: 255, b: 0, value: 0x05 },
    { name: "橙色", realR: 255, realG: 127, realB: 30, r: 255, g: 128, b: 0, value: 0x06 }
  ],
  // sevenColor: [
  //     { name: "黑色", realR: 30, realG: 30, realB: 30, r: 0, g: 0, b: 0, value: 0x00 },
  //     { name: "白色", realR: 180, realG: 180, realB: 180, r: 255, g: 255, b: 255, value: 0x01 },
  //     { name: "绿色", realR: 0, realG: 190, realB: 0, r: 0, g: 255, b: 0, value: 0x02 },
  //     { name: "蓝色", realR: 0, realG: 0, realB: 190, r: 0, g: 0, b: 255, value: 0x03 },
  //     { name: "红色", realR: 185, realG: 0, realB: 0, r: 255, g: 0, b: 0, value: 0x04 },
  //     { name: "黄色", realR: 200, realG: 200, realB: 0, r: 255, g: 255, b: 0, value: 0x05 },
  //     { name: "橙色", realR: 220, realG: 150, realB: 30, r: 255, g: 128, b: 0, value: 0x06 }
  // ], 
  sixColor: [
...
/**
 * 增强版颜色匹配函数 – 解决六色/七色屏蓝绿色偏黄问题
 * 策略：硬规则优先（利用RGB通道差异识别特定颜色），再回退至CIELAB色差计算
 * @param {number} r 红色分量 0-255
 * @param {number} g 绿色分量 0-255
 * @param {number} b 蓝色分量 0-255
 * @param {string} mode 颜色模式: 'blackWhiteColor', 'threeColor', 'fourColor', 'sixColor', 'sevenColor'
 * @returns {object} 匹配的颜色对象 {r, g, b, value}
 */
//调整灰度与多色灰度的取值
function findClosestColor(r, g, b, mode) {
  ...
  if (mode === 'sixColor') {
    ...
    // 低饱和度（灰色）→ 直接返回黑白，避免混入彩色
    if (saturation < 0.15) {
      return luminance < 140
        ? (epdSixColorWithLab.find(c => c.value === 0x00) || epdSixColorWithLab[0])
        : (epdSixColorWithLab.find(c => c.value === 0xFF) || epdSixColorWithLab[5]);
    }

    // ---- 硬规则优先（高饱和色） ----
    // 蓝色: B通道明显高于R/G，且B > 100，R/G较低
    // if (b > 100 && b > g + 20 && b > r + 20 && (r < 130 || g < 130)) {
    //   return epdSixColorWithLab.find(c => c.value === 0x1D) || epdSixColorWithLab[3];
    // }
    // // 绿色: G通道明显高于R/B，且G > 100
    // if (g > 100 && g > r + 25 && g > b + 25) {
    //     return epdSixColorWithLab.find(c => c.value === 0x96) || epdSixColorWithLab[2];
    // }
    // // 红色: R通道显著高于G/B
    // if (r > 120 && r > g * 1.4 && r > b * 1.4) {
    //     return epdSixColorWithLab.find(c => c.value === 0x4C) || epdSixColorWithLab[4];
    // }
    // // 黄色: R和G都高，B明显低
    // if (r > 150 && g > 150 && b < 120 && (r + g) > (b + 80)) {
    //     return epdSixColorWithLab.find(c => c.value === 0xE2) || epdSixColorWithLab[1];
    // }

    // // 亮度极端时直接返回黑白（避免暗部彩色或亮部彩色过于突出）
    // if (luminance < 100) {
    //     return epdSixColorWithLab.find(c => c.value === 0x00) || epdSixColorWithLab[0];
    // }
    // if (luminance > 220) {
    //     return epdSixColorWithLab.find(c => c.value === 0xFF) || epdSixColorWithLab[5];
    // }
  ...
  if (mode === 'sevenColor') {
    ...
    if (saturation < 0.15) {
      return luminance < 140
        ? (epdSevenColorWithLab.find(c => c.value === 0x00) || epdSevenColorWithLab[0])
        : (epdSevenColorWithLab.find(c => c.value === 0x01) || epdSevenColorWithLab[1]);
    }

    // ---- 硬规则优先（高饱和色） ----
    // 蓝色: B通道显著高
    // if (b > 100 && b > g + 20 && b > r + 20 && (r < 150 || g < 150)) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x03) || epdSevenColorWithLab[5];
    // }
    // // 绿色: G通道显著高
    // if (g > 100 && g > r + 25 && g > b + 25) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x02) || epdSevenColorWithLab[4];
    // }
    // // 红色: R通道显著高
    // if (r > 120 && r > g * 1.4 && r > b * 1.4) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x04) || epdSevenColorWithLab[2];
    // }
    // // 黄色: R和G高，B低
    // if (r > 150 && g > 150 && b < 120) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x05) || epdSevenColorWithLab[3];
    // }
    // // 橙色: R高，G中等，B低，且R显著高于G
    // if (r > 160 && g > 70 && g < r - 30 && b < 100) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x06) || epdSevenColorWithLab[6];
    // }

    // // 亮度极端时直接返回黑白
    // if (luminance < 100) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x00) || epdSevenColorWithLab[0];
    // }
    // if (luminance > 220) {
    //     return epdSevenColorWithLab.find(c => c.value === 0x01) || epdSevenColorWithLab[1];
    // }
...
//新改的, 看着准确度更高
function _findClosestColor(r, g, b, mode) {
  if (mode === 'sevenColor') {
    // --- 新增：紫色检测逻辑 ---
    const redness = r / (Math.max(g, b) + 1); // +1 防止除零
    const blueness = b / (Math.max(g, r) + 1);
    // 如果红色和蓝色都显著高于绿色，且红蓝强度相近，则认为是紫色区域
    if (r > 50 && b > 50 && g < (r + b) * 0.5 && Math.abs(redness - blueness) < 0.5) {
      // 紫色区域，优先在红色和蓝色之间选择
      // 可以根据全局抖动误差或其他因素来决定选红还是选蓝
      // 这里简化处理，可以根据需要调整
      const targetLab = rgbToLabRed(r, g, b);
      let redColor = epdSevenColorWithLab.find(c => c.value === 0x04); // 红色
      let blueColor = epdSevenColorWithLab.find(c => c.value === 0x03); // 蓝色
      if (!redColor || !blueColor) {
        // 如果找不到红蓝定义，则回退到原始逻辑
        console.warn("Warning: Red or Blue color definition not found for purple detection.");
        // 执行原始逻辑
        const targetLab = rgbToLabRed(r, g, b);
        let best = epdSevenColorWithLab[0];
        let bestDist = Infinity;
      ...
      // 简单策略：选择红色和蓝色中与目标色差较小的那个
      // （或者可以实现更复杂的策略，例如基于当前像素坐标的棋盘格模式）
      const distToRed = ciede2000(targetLab, redColor.lab);
      const distToBlue = ciede2000(targetLab, blueColor.lab);
      // 如果距离相近，可以考虑其他因素，比如让相邻像素交替选择
      // 这里先用简单的距离比较
      return distToRed <= distToBlue ? redColor : blueColor;
    }
    // --- 紫色检测逻辑结束 ---
  ...
  for (let rIdx = 0; rIdx < size; rIdx++) {
    ...
    for (let gIdx = 0; gIdx < size; gIdx++) {
      ...
      for (let bIdx = 0; bIdx < size; bIdx++) {
        ...
        lut[baseIndex + bIdx] = _findClosestColor(r, g, b, type);
      }
    }
  }
  return lut;
...
function errorDiffusionDither(imageData, strength, colorMode, kernel) {
  ...
  // 判断是否为青色特征（用于七色/六色模式）
  function isCyan(r8, g8, b8) {
    if (colorMode !== 'sevenColor' && colorMode !== 'sixColor') return false;
    return (r8 < 100 && g8 > 120 && b8 > 120 && Math.abs(g8 - b8) < 60);
  }
  function isMagenta(r8, g8, b8) {
    if (colorMode !== 'sevenColor' && colorMode !== 'sixColor') return false;
    return (r8 > 150 && b8 > 120 && g8 < 100 && Math.abs(r8 - b8) < 60);
  }

  // 获取虚拟色对应的两个真实颜色（青色:蓝+绿; 洋红:红+蓝）
  function getDitherPair(r8, g8, b8) {
    if (isCyan(r8, g8, b8)) {
      const blue = (colorMode === 'sixColor')
        ? epdSixColorWithLab.find(c => c.value === 0x1D)
        : epdSevenColorWithLab.find(c => c.value === 0x03);
      const green = (colorMode === 'sixColor')
        ? epdSixColorWithLab.find(c => c.value === 0x96)
        : epdSevenColorWithLab.find(c => c.value === 0x02);
      return { colorA: blue, colorB: green, ratio: 0.5 }; // 默认各半
    }
    if (isMagenta(r8, g8, b8)) {
      const red = (colorMode === 'sixColor')
        ? epdSixColorWithLab.find(c => c.value === 0x4C)
        : epdSevenColorWithLab.find(c => c.value === 0x04);
      const blue = (colorMode === 'sixColor')
        ? epdSixColorWithLab.find(c => c.value === 0x1D)
        : epdSevenColorWithLab.find(c => c.value === 0x03);
      return { colorA: red, colorB: blue, ratio: 0.5 };
    }
    return null;
  }
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = xStart; x !== xEnd; x += step) {
      const idx = y * width + x;
      const rLin = linearR[idx];
      const gLin = linearG[idx];
      const bLin = linearB[idx];
      const r8 = _linearToSrgb[Math.min(4095, Math.max(0, 4095 * rLin + 0.5) | 0)];
      const g8 = _linearToSrgb[Math.min(4095, Math.max(0, 4095 * gLin + 0.5) | 0)];
      const b8 = _linearToSrgb[Math.min(4095, Math.max(0, 4095 * bLin + 0.5) | 0)];

      let closest;
      const ditherPair = getDitherPair(r8, g8, b8);
      if (ditherPair) {
        // 根据坐标奇偶选择颜色 A 或 B（可调整比例）
        const useA = ((x + y) & 1) === 0;
        closest = useA ? ditherPair.colorA : ditherPair.colorB;
      } else {
        closest = document.getElementById('useLegacyDither').checked
          ? findClosestColorRed(r8, g8, b8, colorMode)
          : findClosestColor(r8, g8, b8, colorMode);
      }
...
function errorDiffusionDither(imageData, strength, colorMode, kernel) {
  ...
  // 获取虚拟色对应的两个真实颜色（洋红:红+蓝）
  function getMagentaPair(r8, g8, b8) {
    const red = (colorMode === 'sixColor')
      ? epdSixColorWithLab.find(c => c.value === 0x4C)
      : epdSevenColorWithLab.find(c => c.value === 0x04);
    const blue = (colorMode === 'sixColor')
    ...
    for (let x = xStart; x !== xEnd; x += step) {
      ...
      if (isCyan(r8, g8, b8)) {
        ...
        const green = (colorMode === 'sixColor')
          ? epdSixColorWithLab.find(c => c.value === 0x96)
          : epdSevenColorWithLab.find(c => c.value === 0x02);
        const white = (colorMode === 'sixColor')
          ? epdSixColorWithLab.find(c => c.value === 0xFF)
          : epdSevenColorWithLab.find(c => c.value === 0x01);
        // 蓝:绿:白 = 1:1:1 → 使用 (x+y) % 3
        const mod = (x + y) % 3;
        if (mod === 0) closest = blue;
        else if (mod === 1) closest = green;
        else closest = white;
      }
      // 处理洋红色（红蓝各半）
      else if (isMagenta(r8, g8, b8)) {
        const pair = getMagentaPair(r8, g8, b8);
        const useRed = ((x + y) & 1) === 0;
        closest = useRed ? pair.colorA : pair.colorB;
      }
      // 其他颜色正常匹配
      else {
        closest = document.getElementById('useLegacyDither').checked
...
// ==================== 具体抖动算法实现 ====================
function floydSteinbergDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, FLOYD_STEINBERG_KERNEL);
}

function atkinsonDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, ATKINSON_KERNEL);
}

function stuckiDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, STUCKI_KERNEL);
}

function jarvisDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, JARVIS_KERNEL);
}
...
function linearLightSierraDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, LINEAR_LIGHT_SIERRA_KERNEL);
}

function diagonalDither(imageData, strength, colorMode) {
  return errorDiffusionDither(imageData, strength, colorMode, DIAGONAL_KERNEL);
}
 ...
 */
function linearLightSierraDither66(imageData, strength, colorMode) {
  const width = imageData.width, height = imageData.height;
  const data = imageData.data;
  const pixelCount = width * height;
  const linearR = new Float32Array(pixelCount);
